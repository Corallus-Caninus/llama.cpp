...
extern "C"{
	...
	struct trainer init_trainer_params() ;
...
struct finetune_llama_model {
    ...
    struct ggml_tensor *norm; // presumably the output layers normalization TODO verify
...
static void load_model_hparams_gguf(struct gguf_context *ctx, struct finetune_llama_hparams *hparams, const char *expected_arch) {
    std::string arch;
...
static void init_model(struct llama_model *input, struct finetune_llama_model *model, const char *fn_model, uint32_t n_ctx) {
...
static void init_lora(const struct finetune_llama_model *model, struct finetune_llama_lora *lora) {
...
static void randomize_lora(struct finetune_llama_lora *lora, int seed, float mean, float std, float min, float max) {
    const uint32_t n_layer = lora->layers.size();
...
static struct ggml_tensor *llama_build_lora_finetune_graphs(struct finetune_llama_model *model, struct finetune_llama_lora *lora, ggml_gallocr_t alloc, struct ggml_context *ctx, struct ggml_cgraph *gf, struct ggml_cgraph *gb, struct ggml_cgraph *gb_tmp, struct ggml_tensor **logits, struct ggml_tensor *tokens_input, struct ggml_tensor *targets, const int n_tokens, const int n_batch, const bool enable_flash_attn, const bool enable_checkpointing, const bool measure_only) {
    ...
    auto rope = [ ctx, KQ_pos, n_rot, n_ctx, rope_freq_base, rope_freq_scale ](struct ggml_tensor * t) -> struct ggml_tensor * {
        // not capturing these, to silcence warnings
        const int rope_mode = 0;

        return ggml_rope_custom(ctx, t, KQ_pos, n_rot, rope_mode, n_ctx, 0, rope_freq_base, rope_freq_scale, 0.0f, 1.0f, 0.0f, 0.0f);
    };
    ...
    // TODO: should this also happen in quantized types with f32 activations?
    // TODO: if we want to use GPU, LoRa has to do the casting to f32 type with
    // ggml graph.
    auto add_to_f32 = [](struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b) {
        if (ggml_is_quantized(a->type) || a->type == GGML_TYPE_F16 || a->type == GGML_TYPE_Q8_0) {
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        struct ggml_tensor *t06 = ggml_reshape_4d(ctx, t05, n_embd_head, n_head, N, n_batch);
        set_name(t06, "t06");
        ...
        struct ggml_tensor *t09 = ggml_reshape_4d(ctx, t08, n_embd_head, n_head_kv, N, n_batch);
        set_name(t09, "t09");
        ...
        struct ggml_tensor *t12 = ggml_reshape_4d(ctx, t11, N, n_batch, n_embd_head, n_head_kv);
        set_name(t12, "t12");
        ...
        struct ggml_tensor *t25 = ggml_mul_mat(ctx, ffn_up, t24);
        set_name(t25, "t25");
        assert_shape_2d(t25, n_ff, N * n_batch);
        struct ggml_tensor *t26 = ggml_mul_mat(ctx, ffn_gate, t24);
        set_name(t26, "t26");
        ...
        struct ggml_tensor *t29 = ggml_mul_mat(ctx, ffn_down, t28);
        set_name(t29, "t29");
    ...
    // make sure some tensors are not reallocated by inserting new temporary nodes
    // depending on them
...
static void load_llama_lora_gguf(struct gguf_context *fctx, struct ggml_context *f_ggml_ctx, struct finetune_llama_model *model, struct finetune_llama_lora *lora) {
...
static void save_llama_lora_gguf(struct gguf_context *fctx, struct finetune_llama_model *model, struct finetune_llama_lora *lora) {
...
static void load_checkpoint_lora_gguf(struct gguf_context *fctx, struct ggml_context *f_ggml_ctx, struct finetune_llama_model *model, struct finetune_llama_lora *lora, struct train_state *train) {
...
static void save_checkpoint_lora_gguf(struct gguf_context *fctx, struct finetune_llama_model *model, struct finetune_llama_lora *lora, struct train_state *train) {
...
// TODO: create a struct for holding and initializing lora params
// TODO: name this train_config and move all the initialization into get_default
struct train_params {
    // TODO: bring in everything from int main
    // TODO: bring in llama_mparams, train_state and ggml_opt_context here.
...
// TODO: PROTOTYPE, eventually this all needs aggressive refactoring once a
// basic library is realized
// TODO: checkpointing should be optional, just save LATEST
// TODO: config struct (with consolidation of these structs?)
// TODO: state struct
// TODO: all the extraction goes in those two structs^
// glue structure to organize the disparate parts of finetune
struct trainer {
...
void load_checkpoint_lora_file(const char *filename, struct trainer *trainer) {
    ...
    if (fctx == NULL) {
        ...
        randomize_lora(&trainer->lora, trainer->params.common.seed, 0.0f, 1.0f, -1.0f, +1.0f);
        return;
        // TODO: DCE
        ...
        //    }
        //  }
        ggml_opt_init(trainer->train->opt->ctx, trainer->train->opt, trainer->train->opt->params, get_parameter_count(&trainer->lora));
    }
    ...
    const bool opt_param_count_changed = 
		((trainer->lora.hparams.n_rank_attention_norm != prev_trainer.lora.hparams.n_rank_attention_norm) ||
		 ...
		 (trainer->lora.hparams.n_rank_tok_embeddings != prev_trainer.lora.hparams.n_rank_tok_embeddings) ||
...
static void save_checkpoint_lora_file(const char *filename, struct finetune_llama_model *model, struct finetune_llama_lora *lora, struct train_state *train) {
...
static void save_as_llama_lora(const char *filename, struct finetune_llama_lora *lora) {
    printf("%s: saving to %s\n", __func__, filename);
...
//      exit\n");
//
//      fprintf(stderr, "  --model-base FNAME         model path from which to
//      load base model (default '%s')\n", params->fn_model_base);
//      fprintf(stderr, "  --lora-out FNAME           path to save llama lora
//      (default '%s')\n", params->fn_lora_out); fprintf(stderr, "
//      --only-write-lora          only save llama lora, don't do any training.
//      use this if you only want to convert a checkpoint to a lora adapter.\n");
//      fprintf(stderr, "  --norm-rms-eps F           RMS-Norm epsilon value
//      (default %f)\n", params->f_norm_rms_eps); fprintf(stderr, "
//      --rope-freq-base F         Frequency base for ROPE (default %f)\n",
//      params->rope_freq_base); fprintf(stderr, "  --rope-freq-scale F Frequency
//      scale for ROPE (default %f)\n", params->rope_freq_scale); fprintf(stderr,
//      "  --lora-alpha N             LORA alpha : resulting LORA scaling is
//      alpha/r. (default %d)\n", params->lora_alpha); fprintf(stderr, " --lora-r
//      N                 LORA r: default rank. Also specifies resulting scaling
//      together with lora-alpha. (default %d)\n", params->lora_r);
//      fprintf(stderr, "  --rank-att-norm N          LORA rank for attention
//      norm tensor, overrides default rank. Norm tensors should generally have
//      rank 1.\n"); fprintf(stderr, "  --rank-ffn-norm N          LORA rank for
//      feed-forward norm tensor, overrides default rank. Norm tensors should
//      generally have rank 1.\n"); fprintf(stderr, "  --rank-out-norm N LORA
//      rank for output norm tensor, overrides default rank. Norm tensors should
//      generally have rank 1.\n"); fprintf(stderr, "  --rank-tok-embd N LORA
//      rank for token embeddings tensor, overrides default rank.\n");
//      fprintf(stderr, "  --rank-out N               LORA rank for output
//      tensor, overrides default rank.\n"); fprintf(stderr, "  --rank-wq N LORA
//      rank for wq tensor, overrides default rank.\n"); fprintf(stderr, "
//      --rank-wk N                LORA rank for wk tensor, overrides default
//      rank.\n"); fprintf(stderr, "  --rank-wv N                LORA rank for wv
//      tensor, overrides default rank.\n"); fprintf(stderr, "  --rank-wo N LORA
//      rank for wo tensor, overrides default rank.\n"); fprintf(stderr, "
//      --rank-ffn_gate N          LORA rank for ffn_gate tensor, overrides
//      default rank.\n"); fprintf(stderr, "  --rank-ffn_down N          LORA
//      rank for ffn_down tensor, overrides default rank.\n"); fprintf(stderr, "
//      --rank-ffn_up N            LORA rank for ffn_up tensor, overrides default
//      rank.\n");
//
//      print_common_train_usage(argc, argv, &params->common);
//  }
//  static bool train_params_parse(int argc, char ** argv, struct train_params *
//  params) {
...
void initialize_lora_hparams(struct trainer *trainer) {
    ...
    if (trainer->params.custom_rope_freq_base) {
        trainer->model.hparams.rope_freq_base = trainer->params.rope_freq_base;
    }
    if (trainer->params.custom_rope_freq_scale) {
        trainer->model.hparams.rope_freq_scale = trainer->params.rope_freq_scale;
    }
    trainer->lora.hparams.lora_r                = trainer->params.lora_r;
    trainer->lora.hparams.lora_alpha            = trainer->params.custom_lora_alpha ? trainer->params.lora_alpha : trainer->params.lora_r;
    uint32_t n_rank_attention_norm              = trainer->params.custom_n_rank_attention_norm ? trainer->params.n_rank_attention_norm : 1;
    uint32_t n_rank_wq                          = trainer->params.custom_n_rank_wq ? trainer->params.n_rank_wq : trainer->params.lora_r;
    uint32_t n_rank_wk                          = trainer->params.custom_n_rank_wk ? trainer->params.n_rank_wk : trainer->params.lora_r;
    uint32_t n_rank_wv                          = trainer->params.custom_n_rank_wv ? trainer->params.n_rank_wv : trainer->params.lora_r;
    uint32_t n_rank_wo                          = trainer->params.custom_n_rank_wo ? trainer->params.n_rank_wo : trainer->params.lora_r;
    uint32_t n_rank_ffn_norm                    = trainer->params.custom_n_rank_ffn_norm ? trainer->params.n_rank_ffn_norm : 1;
    uint32_t n_rank_ffn_gate                    = trainer->params.custom_n_rank_ffn_gate ? trainer->params.n_rank_ffn_gate : trainer->params.lora_r;
    uint32_t n_rank_ffn_down                    = trainer->params.custom_n_rank_ffn_down ? trainer->params.n_rank_ffn_down : trainer->params.lora_r;
    uint32_t n_rank_ffn_up                      = trainer->params.custom_n_rank_ffn_up ? trainer->params.n_rank_ffn_up : trainer->params.lora_r;
    uint32_t n_rank_tok_embeddings              = trainer->params.custom_n_rank_tok_embeddings ? trainer->params.n_rank_tok_embeddings : trainer->params.lora_r;
    uint32_t n_rank_norm                        = trainer->params.custom_n_rank_norm ? trainer->params.n_rank_norm : 1;
    uint32_t n_rank_output                      = trainer->params.custom_n_rank_output ? trainer->params.n_rank_output : trainer->params.lora_r;
    trainer->lora.hparams.n_rank_attention_norm = n_rank_attention_norm;
    trainer->lora.hparams.n_rank_wq             = n_rank_wq;
    trainer->lora.hparams.n_rank_wk             = n_rank_wk;
    trainer->lora.hparams.n_rank_wv             = n_rank_wv;
    trainer->lora.hparams.n_rank_wo             = n_rank_wo;
    trainer->lora.hparams.n_rank_ffn_norm       = n_rank_ffn_norm;
    trainer->lora.hparams.n_rank_ffn_gate       = n_rank_ffn_gate;
    trainer->lora.hparams.n_rank_ffn_down       = n_rank_ffn_down;
    trainer->lora.hparams.n_rank_ffn_up         = n_rank_ffn_up;
    trainer->lora.hparams.n_rank_tok_embeddings = n_rank_tok_embeddings;
...
struct trainer initialize_trainer() {
    ...
    init_model(trainer.lmodel, &trainer.model, trainer.params.fn_model_base, trainer.params.common.n_ctx);
    trainer.train = init_train_state();
    // dropped opt here in favor of referencing trainer.train->opt for clarity
    trainer.train->opt->params.print_forward_graph  = true;
    trainer.train->opt->params.print_backward_graph = true;
    initialize_lora_hparams(&trainer);
    ...
    trainer.train->opt->params.max_no_improvement = trainer.params.common.opt_max_no_improvement;
    ...
    trainer.train->opt->params.lbfgs.linesearch = GGML_LINESEARCH_BACKTRACKING_STRONG_WOLFE;
    printf("%s: init model\n", __func__);
    ...
    print_lora_params(&trainer.lora.hparams);
    printf("%s: total train_iterations %llu\n", __func__, (long long unsigned)trainer.train->train_its);
    printf("%s: seen train_samples     %llu\n", __func__, (long long unsigned)trainer.train->train_samples);
    printf("%s: seen train_tokens      %llu\n", __func__, (long long unsigned)trainer.train->train_tokens);
    printf("%s: completed train_epochs %llu\n", __func__, (long long unsigned)trainer.train->train_epochs);
    printf("%s: lora_size = %zu bytes (%.1f MB)\n", __func__, (ggml_used_mem(trainer.lora.ctx) + ggml_backend_buffer_get_size(trainer.lora.data)), (float)(ggml_used_mem(trainer.lora.ctx) + ggml_backend_buffer_get_size(trainer.lora.data)) / (1024.0f * 1024.0f));
    printf("%s: opt_size  = %zu bytes (%.1f MB)\n", __func__, ggml_get_mem_size(trainer.train->opt->ctx), (float)ggml_get_mem_size(trainer.train->opt->ctx) / (1024.0f * 1024.0f));
    printf("%s: opt iter %d\n", __func__, trainer.train->opt->iter);
    int n_tokens     = trainer.model.hparams.n_ctx;
    trainer.n_tokens = n_tokens;
    int n_vocab      = trainer.model.hparams.n_vocab;
    int n_batch      = trainer.params.common.n_batch;
    ...
    struct ggml_tensor *tokens_input = ggml_new_tensor_2d(ctx_input, GGML_TYPE_I32, trainer.n_tokens, n_batch);
    struct ggml_tensor *target_probs = ggml_new_tensor_3d(ctx_input, GGML_TYPE_F32, n_vocab, trainer.n_tokens, n_batch);
    ...
    const size_t            estimated_compute_size_wo_data = (2 * LLAMA_TRAIN_MAX_NODES * ggml_tensor_overhead() + (trainer.params.common.use_checkpointing ? 3 : 2) * (GGML_OBJECT_SIZE + ggml_graph_overhead_custom(LLAMA_TRAIN_MAX_NODES, true)));
    ...
    for (unsigned order = 0; order < (unsigned)GGML_CGRAPH_EVAL_ORDER_COUNT; ++order) {
        ...
        gb_tmp                  = trainer.params.common.use_checkpointing ? ggml_new_graph_custom(ctx_compute, LLAMA_TRAIN_MAX_NODES, true) : NULL;
        loss                    = llama_build_lora_finetune_graphs(&trainer.model, &trainer.lora, alloc, ctx_compute, gf, gb, gb_tmp, &logits, tokens_input, target_probs, trainer.n_tokens, n_batch, trainer.params.common.use_flash, trainer.params.common.use_checkpointing, true);
    ...
    printf("%s: evaluation order = %s\n", __func__, (best_order == GGML_CGRAPH_EVAL_ORDER_LEFT_TO_RIGHT) ? "LEFT_TO_RIGHT" : (best_order == GGML_CGRAPH_EVAL_ORDER_RIGHT_TO_LEFT) ? "RIGHT_TO_LEFT" : "invalid");
    ...
    gb                   = ggml_new_graph_custom(ctx_compute, LLAMA_TRAIN_MAX_NODES, true);
    gb_tmp               = trainer.params.common.use_checkpointing ? ggml_new_graph_custom(ctx_compute, LLAMA_TRAIN_MAX_NODES, true) : NULL;
    loss                 = llama_build_lora_finetune_graphs(&trainer.model, &trainer.lora, alloc, ctx_compute, gf, gb, gb_tmp, &logits, tokens_input, target_probs, trainer.n_tokens, n_batch, trainer.params.common.use_flash, trainer.params.common.use_checkpointing, false);
    trainer.loss         = loss;
    trainer.gb           = gb;
    trainer.gf           = gf;
    return trainer;
...
int trainer_execute(struct trainer trainer) {
    ...
    // TODO: extract this, can be used to tokenize while model is loaded as a
    // trainning daemon
    std::vector<llama_token> train_tokens;
    std::vector<size_t>      train_samples_begin;
    std::vector<size_t>      train_samples_size;
    printf("%s: tokenize training data from %s\n", __func__, trainer.params.common.fn_train_data);
    printf("%s: sample-start: %s\n", __func__, trainer.params.common.sample_start.c_str());
    printf("%s: include-sample-start: %s\n", __func__, trainer.params.common.include_sample_start ? "true" : "false");
    printf("%s: overlapping-samples: %s\n", __func__, trainer.params.common.overlapping_samples ? "true" : "false");
    // trainer.n_tokens
    printf("%s: n_tokens: %d\n", __func__, trainer.n_tokens);
    tokenize_file(trainer.lctx, trainer.params.common.fn_train_data, trainer.params.common.sample_start, trainer.params.common.include_sample_start, trainer.params.common.overlapping_samples, trainer.n_tokens, train_tokens, train_samples_begin, train_samples_size);
    GGML_ASSERT(train_samples_begin.size() == train_samples_size.size());
    printf("%s: number of training tokens: %zu\n", __func__, train_tokens.size());
    std::vector<size_t> token_noccurs;
    token_noccurs.resize(trainer.model.hparams.n_vocab, 0);
    ...
    size_t     shuffle_samples_hash = compute_samples_hash(trainer.params.common.fn_train_data, train_samples_begin.data(), train_samples_size.data(), train_samples_size.size());
    const bool changed_train_data   = (shuffle_samples_hash != trainer.train->shuffle_samples_hash) || (trainer.train->shuffle_sample_count != train_samples_size.size());
    if (changed_train_data) {
        printf("%s: train data seems to have changed. restarting shuffled epoch.\n", __func__);
    }
    if (trainer.params.common.force_reshuffle) {
        printf("%s: forced reshuffling of data. restarting with newly shuffled "
               "epoch.\n",
               __func__);
    }
    if ((trainer.train->shuffle_rng_state_current == "") || changed_train_data || trainer.params.common.force_reshuffle) {
        trainer.train->shuffle_rng_state_current = mt19937_seed_to_state(trainer.params.common.seed);
        trainer.train->shuffle_sample_count      = train_samples_size.size();
        trainer.train->shuffle_next_sample       = 0;
        trainer.train->shuffle_samples_hash      = shuffle_samples_hash;
    }
    ...
    trainer.train->shuffle_rng_state_next = shuffle_samples(trainer.train->shuffle_rng_state_current, train_shuffled_samples_offs.data(), train_shuffled_samples_begin.data(), train_shuffled_samples_size.data(), train_samples_begin.data(), train_samples_size.data(), train_samples_size.size());
    printf("%s: begin training\n", __func__);
    save_train_files_data save_data;
    save_data.fn_checkpoint_out = trainer.params.common.fn_checkpoint_out;
    ...
    opt_cb_data.params                 = &trainer.params.common;
    opt_cb_data.train                  = trainer.train;
    opt_cb_data.save_cb                = &save_train_files;
    opt_cb_data.save_data              = &save_data;
    opt_cb_data.lctx                   = trainer.lctx;
    opt_cb_data.last_save_iter         = trainer.train->opt->iter;
    ...
    opt_cb_data.tokens_input           = trainer.tokens_input;
    opt_cb_data.target_probs           = trainer.target_probs;
    opt_cb_data.first_iter             = trainer.train->opt->iter;
    opt_cb_data.first_epoch            = trainer.train->train_epochs;
    opt_cb_data.iter_at_last_epoch     = -1;
    opt_cb_data.last_time              = ggml_time_ms();
    opt_cb_data.millis_per_iter        = 0.0;
    // measure required memory for work buffer
    size_t max_work_size = ggml_graph_plan(trainer.gb, trainer.params.common.n_threads).work_size + GGML_OBJECT_SIZE;
    ...
    ggml_opt_resume_g(ctx_work, trainer.train->opt, trainer.loss, trainer.gf, trainer.gb, &train_opt_callback, (void *)&opt_cb_data);
    // TODO remove this to a seperate function
    // ggml_free(ctx_work);
    // ggml_free(ctx_compute);
    // ggml_free(ctx_input);
    // ggml_gallocr_free(alloc);
    ...
    printf("\n");
    int new_iters = trainer.train->opt->iter - opt_cb_data.last_save_iter;
    if (new_iters > 0) {
        trainer.train->train_its += new_iters;
        trainer.train->train_tokens += new_iters * trainer.train->opt->params.n_gradient_accumulation * trainer.params.common.n_batch * trainer.n_tokens;
