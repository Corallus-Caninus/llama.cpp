...
static void load_model_hparams_gguf(struct gguf_context *ctx,
  ...
  GGUF_GET_KEY(ctx, rope_freq_scale, gguf_get_val_f32, GGUF_TYPE_FLOAT32, false,
...
static void init_lora(const struct finetune_llama_model *model,
  ...
  for (uint32_t i = 0; i < n_layer; ++i) {
    ...
    layer.wk_b =
	ggml_new_tensor_2d(ctx, GGML_TYPE_F32, lparams.n_rank_wk, n_embd_gqa);
    layer.wv_a =
	ggml_new_tensor_2d(ctx, GGML_TYPE_F32, lparams.n_rank_wv, n_embd);
    layer.wv_b =
	ggml_new_tensor_2d(ctx, GGML_TYPE_F32, lparams.n_rank_wv, n_embd_gqa);
    layer.wo_a =
    ...
    layer.ffn_norm_a =
	ggml_new_tensor_2d(ctx, GGML_TYPE_F32, lparams.n_rank_ffn_norm, n_embd);
    layer.ffn_norm_b =
	ggml_new_tensor_2d(ctx, GGML_TYPE_F32, lparams.n_rank_ffn_norm, 1);

    layer.ffn_gate_a =
	ggml_new_tensor_2d(ctx, GGML_TYPE_F32, lparams.n_rank_ffn_gate, n_embd);
    layer.ffn_gate_b =
	ggml_new_tensor_2d(ctx, GGML_TYPE_F32, lparams.n_rank_ffn_gate, n_ff);
    layer.ffn_down_a =
	ggml_new_tensor_2d(ctx, GGML_TYPE_F32, lparams.n_rank_ffn_down, n_ff);
    layer.ffn_down_b =
	ggml_new_tensor_2d(ctx, GGML_TYPE_F32, lparams.n_rank_ffn_down, n_embd);
    layer.ffn_up_a =
	ggml_new_tensor_2d(ctx, GGML_TYPE_F32, lparams.n_rank_ffn_up, n_embd);
    layer.ffn_up_b =
	ggml_new_tensor_2d(ctx, GGML_TYPE_F32, lparams.n_rank_ffn_up, n_ff);
    ...
    ggml_set_name(layer.ffn_gate_a,
		  tni(LLM_TENSOR_FFN_GATE, ".weight.lora_a", i));
    ggml_set_name(layer.ffn_gate_b,
		  tni(LLM_TENSOR_FFN_GATE, ".weight.lora_b", i));
    ggml_set_name(layer.ffn_down_a,
		  tni(LLM_TENSOR_FFN_DOWN, ".weight.lora_a", i));
    ggml_set_name(layer.ffn_down_b,
  ...
  lora->data = ggml_backend_alloc_ctx_tensors_from_buft(
...
static struct ggml_tensor *llama_build_lora_finetune_graphs(
  ...
  auto rope = [ ctx, KQ_pos, n_rot, n_ctx, rope_freq_base, rope_freq_scale ](
    ...
    return ggml_rope_custom(ctx, t, KQ_pos, n_rot, rope_mode, n_ctx, 0,
  ...
  auto add_to_f32 = [](struct ggml_context *ctx, struct ggml_tensor *a,
	...
	a->type == GGML_TYPE_Q8_0) {
      ...
      die_fmt(
	  "%s: Finetuning on tensors with type '%s' is not yet supported.\n",
  ...
  for (int il = 0; il < n_layer; ++il) {
	...
	ggml_mul_mat(ctx, llayer.attention_norm_a, llayer.attention_norm_b));
    struct ggml_tensor *ffn_norm =
	add_to_f32(ctx, layer.ffn_norm,
		   ggml_mul_mat(ctx, llayer.ffn_norm_a, llayer.ffn_norm_b));
    struct ggml_tensor *wq =
	add_to_f32(ctx, layer.wq, ggml_mul_mat(ctx, llayer.wq_a, llayer.wq_b));
    struct ggml_tensor *wk =
	add_to_f32(ctx, layer.wk, ggml_mul_mat(ctx, llayer.wk_a, llayer.wk_b));
    struct ggml_tensor *wv =
	add_to_f32(ctx, layer.wv, ggml_mul_mat(ctx, llayer.wv_a, llayer.wv_b));
    struct ggml_tensor *wo =
	add_to_f32(ctx, layer.wo, ggml_mul_mat(ctx, llayer.wo_a, llayer.wo_b));
    struct ggml_tensor *ffn_gate =
	add_to_f32(ctx, layer.ffn_gate,
		   ggml_mul_mat(ctx, llayer.ffn_gate_a, llayer.ffn_gate_b));
    struct ggml_tensor *ffn_down =
	add_to_f32(ctx, layer.ffn_down,
		   ggml_mul_mat(ctx, llayer.ffn_down_a, llayer.ffn_down_b));
    struct ggml_tensor *ffn_up = add_to_f32(
	ctx, layer.ffn_up, ggml_mul_mat(ctx, llayer.ffn_up_a, llayer.ffn_up_b));
  ...
  ggml_build_forward_expand(
      gb, ggml_scale_inplace(ctx, model->tok_embeddings, 1.0f));
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    ggml_build_forward_expand(gb,
			      ggml_scale_inplace(ctx, layer.ffn_norm, 1.0f));
    ggml_build_forward_expand(gb, ggml_scale_inplace(ctx, layer.wq, 1.0f));
    ggml_build_forward_expand(gb, ggml_scale_inplace(ctx, layer.wk, 1.0f));
    ggml_build_forward_expand(gb, ggml_scale_inplace(ctx, layer.wv, 1.0f));
    ggml_build_forward_expand(gb, ggml_scale_inplace(ctx, layer.wo, 1.0f));
    ggml_build_forward_expand(gb,
			      ggml_scale_inplace(ctx, layer.ffn_gate, 1.0f));
    ggml_build_forward_expand(gb,
			      ggml_scale_inplace(ctx, layer.ffn_down, 1.0f));
...
static void load_llama_lora_gguf(struct gguf_context *fctx,
  ...
  GGUF_GET_KEY(fctx, lora->hparams.n_rank_ffn_gate, gguf_get_val_u32,
	       GGUF_TYPE_UINT32, true, LLM_KV_TRAINING_LORA_RANK_FFN_GATE);
  GGUF_GET_KEY(fctx, lora->hparams.n_rank_ffn_down, gguf_get_val_u32,
  ...
  for (uint32_t i = 0; i < lora->layers.size(); ++i) {
    ...
    copy_tensor_by_name(layer.ffn_gate_a, f_ggml_ctx,
			ggml_get_name(layer.ffn_gate_a));
    copy_tensor_by_name(layer.ffn_gate_b, f_ggml_ctx,
			ggml_get_name(layer.ffn_gate_b));
    copy_tensor_by_name(layer.ffn_down_a, f_ggml_ctx,
			ggml_get_name(layer.ffn_down_a));
    copy_tensor_by_name(layer.ffn_down_b, f_ggml_ctx,
			ggml_get_name(layer.ffn_down_b));
    copy_tensor_by_name(layer.ffn_up_a, f_ggml_ctx,
			ggml_get_name(layer.ffn_up_a));
    copy_tensor_by_name(layer.ffn_up_b, f_ggml_ctx,
...
static void save_as_llama_lora(const char *filename,
  ...
  for (uint32_t i = 0; i < lora->layers.size(); ++i) {
    ...
    write_tensor(&file, layer.ffn_gate_a,
		 tni(LLM_TENSOR_FFN_GATE, i, ".weight.loraA"));
    write_tensor(&file, layer.ffn_gate_b,
		 tni(LLM_TENSOR_FFN_GATE, i, ".weight.loraB"));
    write_tensor(&file, layer.ffn_down_a,
		 tni(LLM_TENSOR_FFN_DOWN, i, ".weight.loraA"));
    write_tensor(&file, layer.ffn_down_b,
		 tni(LLM_TENSOR_FFN_DOWN, i, ".weight.loraB"));
    write_tensor(&file, layer.ffn_up_a,
		 tni(LLM_TENSOR_FFN_UP, i, ".weight.loraA"));
    write_tensor(&file, layer.ffn_up_b,
...
void initialize_lora_hparams(struct trainer *trainer) {
  ...
  uint32_t n_rank_attention_norm = trainer->params.custom_n_rank_attention_norm
				       ...
				       : 1;
  uint32_t n_rank_wq = trainer->params.custom_n_rank_wq
			   ? trainer->params.n_rank_wq
			   : trainer->params.lora_r;
  uint32_t n_rank_wk = trainer->params.custom_n_rank_wk
			   ? trainer->params.n_rank_wk
			   : trainer->params.lora_r;
  uint32_t n_rank_wv = trainer->params.custom_n_rank_wv
			   ? trainer->params.n_rank_wv
			   : trainer->params.lora_r;
  uint32_t n_rank_wo = trainer->params.custom_n_rank_wo
			   ? trainer->params.n_rank_wo
			   : trainer->params.lora_r;
  uint32_t n_rank_ffn_norm = trainer->params.custom_n_rank_ffn_norm
				 ? trainer->params.n_rank_ffn_norm
				 : 1;
  uint32_t n_rank_ffn_gate = trainer->params.custom_n_rank_ffn_gate
				 ? trainer->params.n_rank_ffn_gate
				 : trainer->params.lora_r;
  uint32_t n_rank_ffn_down = trainer->params.custom_n_rank_ffn_down
				 ? trainer->params.n_rank_ffn_down
				 : trainer->params.lora_r;
  uint32_t n_rank_ffn_up = trainer->params.custom_n_rank_ffn_up
			       ? trainer->params.n_rank_ffn_up
			       : trainer->params.lora_r;
  uint32_t n_rank_tok_embeddings = trainer->params.custom_n_rank_tok_embeddings
				       ? trainer->params.n_rank_tok_embeddings
				       : trainer->params.lora_r;
  uint32_t n_rank_norm =
      trainer->params.custom_n_rank_norm ? trainer->params.n_rank_norm : 1;
  uint32_t n_rank_output = trainer->params.custom_n_rank_output
			       ? trainer->params.n_rank_output
			       : trainer->params.lora_r;
...
struct trainer initialize_trainer() {
  ...
  printf("%s: total train_iterations %llu\n", __func__,
	 (long long unsigned)trainer.train->train_its);
  printf("%s: seen train_samples     %llu\n", __func__,
	 (long long unsigned)trainer.train->train_samples);
  printf("%s: seen train_tokens      %llu\n", __func__,
	 (long long unsigned)trainer.train->train_tokens);
  printf("%s: completed train_epochs %llu\n", __func__,
	 (long long unsigned)trainer.train->train_epochs);
  printf("%s: lora_size = %zu bytes (%.1f MB)\n", __func__,
	 (ggml_used_mem(trainer.lora.ctx) +
	  ggml_backend_buffer_get_size(trainer.lora.data)),
	 (float)(ggml_used_mem(trainer.lora.ctx) +
		 ggml_backend_buffer_get_size(trainer.lora.data)) /
	     (1024.0f * 1024.0f));
  ...
  printf(
      ...
      ggml_get_mem_size(trainer.train->opt->ctx),
      (float)ggml_get_mem_size(trainer.train->opt->ctx) / (1024.0f * 1024.0f));
  ...
  const size_t estimated_compute_size_wo_data =
      (2 * LLAMA_TRAIN_MAX_NODES * ggml_tensor_overhead() +
       (trainer.params.common.use_checkpointing ? 3 : 2) *
	   (GGML_OBJECT_SIZE +
  ...
  for (unsigned order = 0; order < (unsigned)GGML_CGRAPH_EVAL_ORDER_COUNT;
    ...
    gb_tmp =
	trainer.params.common.use_checkpointing
  ...
  gf->order = best_order;
  gb = ggml_new_graph_custom(ctx_compute, LLAMA_TRAIN_MAX_NODES, true);
  gb_tmp = trainer.params.common.use_checkpointing
	       ? ggml_new_graph_custom(ctx_compute, LLAMA_TRAIN_MAX_NODES, true)
	       : NULL;
  loss = llama_build_lora_finetune_graphs(
      &trainer.model, &trainer.lora, alloc, ctx_compute, gf, gb, gb_tmp,
...
int trainer_execute(struct trainer trainer) {
  ...
  printf("%s: tokenize training data from %s\n", __func__,
	 trainer.params.common.fn_train_data);
  ...
  size_t shuffle_samples_hash = compute_samples_hash(
      trainer.params.common.fn_train_data, train_samples_begin.data(),
      train_samples_size.data(), train_samples_size.size());
  const bool changed_train_data =
      (shuffle_samples_hash != trainer.train->shuffle_samples_hash) ||
  ...
  if ((trainer.train->shuffle_rng_state_current == "") || changed_train_data ||
      trainer.params.common.force_reshuffle) {
    trainer.train->shuffle_rng_state_current =
	mt19937_seed_to_state(trainer.params.common.seed);
    trainer.train->shuffle_sample_count = train_samples_size.size();
  ...
  trainer.train->shuffle_rng_state_next = shuffle_samples(
      trainer.train->shuffle_rng_state_current,
      train_shuffled_samples_offs.data(), train_shuffled_samples_begin.data(),
      train_shuffled_samples_size.data(), train_samples_begin.data(),
      train_samples_size.data(), train_samples_size.size());
  ...
  size_t max_work_size =
      ggml_graph_plan(trainer.gb, trainer.params.common.n_threads).work_size +
      GGML_OBJECT_SIZE;
  ...
  ggml_opt_resume_g(ctx_work, trainer.train->opt, trainer.loss, trainer.gf,
		    trainer.gb, &train_opt_callback, (void *)&opt_cb_data);
  ...
  if (new_iters > 0) {
    ...
    trainer.train->train_tokens +=
	new_iters * trainer.train->opt->params.n_gradient_accumulation *
	trainer.params.common.n_batch * trainer.n_tokens;
